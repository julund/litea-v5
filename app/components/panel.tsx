...
const short: Locale = { formatDistance: (token, count) => {
    ...
    return (
        <div className="flex flex-col p-4">
            ...
            <div className="grid grid-cols-2 gap-4 text-center md:grid-cols-4">
                <PanelItem>
                    <Counter className="text-4xl font-black font-title" value={stats?.pageViews.count} 
                    // callback={(value: number) => count(value)} 
                    />
                    <span className="text-sm text-gray-500">Total views</span>
                </PanelItem>
                <PanelItem>
                    <Counter className="text-4xl font-black font-title" value={stats?.uniqueVisits.count} 
                    // callback={(value: number) => count(value)} 
                    />
                    <span className="text-sm text-gray-500">Unique visits</span>
                </PanelItem>
                <PanelItem>
                    <Counter className="text-4xl font-black font-title" value={stats?.bounceRate.count} 
                    // callback={(value: number) => percent(value)} 
                    />
                    <span className="text-sm text-gray-500">Bounce rate</span>
                </PanelItem>
                <PanelItem>
                    <Counter className="text-4xl font-black font-title" value={stats?.avgVisitDuration.count} 
                    // config={{ duration: 650 }} 
                    // callback={(value: number) => duration(value)}
                     />
