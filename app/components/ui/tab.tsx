...
const Tab = ({ title, titles, children, className, chevronStyle = false, ...props }: { title?: string; titles?: string[], className?: string; children: React.ReactNode, chevronStyle?: boolean, props?: React.DetailedHTMLProps<React.HTMLAttributes<HTMLDivElement>, HTMLDivElement> }) => {
    ...
    return (
        <div className="p-4 m-2 bg-base-50" {...props}>
            {title && <h3 className="text-base text-base-500">{title}</h3>}
            <span className="flex flex-row items-center mb-2">
                {!chevronStyle ?
                    <span className="flex flex-row flex-wrap flex-shrink gap-2">
                        {titles && titles.map((title, index) =>
                            <span key={index} className="mb-2 text-xs">
                                {(current - 1) == index ?
                                    <div className="px-4 py-2 rounded-md select-none text-base-200 bg-base-600">{title} </div> :
                                    <button className="px-4 py-2 transition-all duration-300 rounded-md select-none text-base-900 bg-base-200 hover:bg-base-300 hover:text-base-800" onClick={() => set(index + 1)}>{title}</button>
                                }
                            </span>
                        )}
                    </span> :
