...
export const NavLink = ({ children, ...props }: { children: ReactNode } & NavLinkProps) => {
    ...
    return (
        <RemixNavLink {...props} prefetch="intent" className={({ isActive }) => classNames("flex items-center gap-2 px-4 py-2 w-full text-xl font-title rounded-sm transition-all duration-500", isActive ? activeClassName : inactiveClassName)}>
            {children}
        </RemixNavLink>
    );
};
